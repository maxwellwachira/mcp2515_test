...
String OBD2Class::decodeDTC(byte highByte, byte lowByte) {
...
String OBD2Class::readDTC(uint8_t mode) {
    String dtcStr;
    ...
    for (int retries = 10; retries > 0; retries--) {
        ...
        CAN.write(0x01); // request data byte length
        CAN.write(mode); // Mode 
        if (CAN.endPacket()) {
            Serial.println("Request sent successfully");
            break;
    ...
    for (unsigned long start = millis(); (millis() - start) < _responseTimeout;) {
        if (CAN.parsePacket() != 0) {
            ...
            long ecuID = CAN.packetId();
            Serial.print("ECU ID: 0x");
            Serial.println(ecuID, HEX);

            int responseByteLength = CAN.read();
            Serial.print("Response Byte Length: ");
            Serial.println(responseByteLength);

            if (responseByteLength == -1) return "No Response Byte Length";

            if(responseByteLength > 7){
              Serial.println("Byte length greater than 7");
              actualByteLength = CAN.read();
              Serial.print("Response Byte Length: ");
              Serial.println(actualByteLength);
            }

            int responseMode = CAN.read();
            Serial.print("Response Mode: ");
            Serial.println(responseMode, HEX);

            if (responseMode != (mode | 0x40)) return "Not OBD Mode response";

            int dtcNumber = CAN.read();
            Serial.print("DTC Number: ");
            Serial.println(dtcNumber);
            ...
            if (dtcNumber == 0) {
                ...
                for (int i = 0; i < 2; i++) {
                    int higherOrderByte = CAN.read();
                    Serial.print("Higher bytes: ");
                    Serial.println(higherOrderByte, HEX);
                   
                    int lowerOrderByte = CAN.read();
                    Serial.print("Lower bytes: ");
                    Serial.println(lowerOrderByte, HEX);
                    

                    String dtcCode = decodeDTC((byte)higherOrderByte, (byte)lowerOrderByte);
                    dtcStr += dtcCode + " ";
                }

                // Check if there are more DTCs available
                if (dtcNumber > 2) {
                    Serial.println("More DTCs available, requesting next chunk"); 

                    //compute dtcArr length
                    int dtcLength = (dtcNumber-2)*2;
                    //create a variable to store dtc aaray
                    byte dtcArr[dtcLength];

                    int countTracker = 0;

                    for (int i = 0; i < ceil(actualByteLength / 7.0); i++) {
                      delay(60);
                      int sendFCSuccess = sendFC();
                      if(!sendFCSuccess) return "Failed to request for CF";

                       // wait for response
                      while (CAN.parsePacket() == 0 ||
                            CAN.read() != (0x21 + i)); // correct sequence number
                      
                      // Process the additional DTCs
                      while (CAN.available()) {
                       if(countTracker < 7){
                          for(int i = 0; i < 7; i++){
                            int dtcByte = CAN.read();
                            if(dtcByte == -1) break;
                            dtcArr[i] = dtcByte;
                            countTracker++;
                          }
                       }else if(countTracker >= 7 && countTracker < 14){
                          for(int i = 7; i < 14; i++){
                            int dtcByte = CAN.read();
                            if(dtcByte == -1) break;
                            dtcArr[i] = dtcByte;
                       ...
                       }else if(countTracker >= 14 && countTracker < 21){
                          for(int i = 14; i < 21; i++){
                            int dtcByte = CAN.read();
                            if(dtcByte == -1) break;
                            dtcArr[i] = dtcByte;
                    ...
                    for(int i = 0; i < dtcLength; i=i+2){
                      String dtcCode = decodeDTC(dtcArr[i], dtcArr[i+1]);
                      dtcStr += dtcCode + " ";
                    }
                }
            }
...
String OBD2Class::getAllDTCs(uint8_t mode){
  String allDtcs;
  const int addressCount = 8;
  Ecu11BitAddresses normalAddresses[] = { ECU1, ECU2, ECU3, ECU4, ECU5, ECU6, ECU7, ECU8 };
  Ecu29BitAddresses extendedAddresses[] = { ECU1_EXTENDED, ECU2_EXTENDED, ECU3_EXTENDED, ECU4_EXTENDED, ECU5_EXTENDED, ECU6_EXTENDED, ECU7_EXTENDED, ECU8_EXTENDED };

  for(int i = 0; i < addressCount; i++){
    if (_useExtendedAddressing) {
      CAN.filterExtended(extendedAddresses[i]);
    } else {
      CAN.filter(normalAddresses[i]);
    }
    Serial.print("Reading DTC for ECU#");
    Serial.println(i + 1);
    delay(60);

    String dtcCode = readDTC(mode);
    allDtcs += dtcCode + " "; 
